...
- [x] The validation-failure Specify turn identifies the conflict as repository-state integration work against the configured target branch, not as a request to add publication conflict-handling behavior.
...
- [x] An Application regression proves the resumed Specify turn receives explicit target-branch integration guidance for a persisted publication rebase conflict.
