...
class InterfaceTests(unittest.TestCase):
    ...
    def test_dashboard_separates_global_active_issues_from_repository_issue_views(
        self,
    ) -> None:
        status, _, body = self.request("GET", "/")
        self.assertEqual(status, 200)
        ...
        self.assertIn("String(issue.repository_id) === selectedRepositoryId", dashboard)
        self.assertIn("String(run.repository_id) === selectedRepositoryId", dashboard)
        self.assertIn("const activeRuns = (currentState.runs || []).filter(activeRun)", dashboard)
        self.assertIn("#repository-filter').addEventListener('change'", dashboard)
