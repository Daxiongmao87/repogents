...
class InterfaceTests(unittest.TestCase):
    ...
    def test_dashboard_contains_issue_queue_and_inline_log(
        ...
        for expected in (
            ...
            "Remove repository",
            "Agent-ready issues",
            "Issues shown here belong only to the selected repository.",
            'id="ready-issues"',
            "Runs",
            ...
            "run => String(run.repository_id) === selectedRepositoryId",
            "renderRepositoryDetail();\n  renderReadyIssues();\n  renderRuns();",
            "!['blocked', 'canceled', 'closed'].includes(String(run.state))",
            "Role prompt",
            'draggable="true"',
            "data-run-select",
            "event.target.closest('#all-active-runs [data-run-select]')",
            "data-force-run",
            "/api/runs/priority",
            "/force",
        ):
