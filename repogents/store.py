...
class Store:
    ...
    def _specification_row(cls, row: sqlite3.Row | None) -> dict[str, Any] | None:
        return cls._decode(
            row,
            json_fields=("acceptance_criteria", "dependencies"),
            bool_fields=("executable",),
        )

    @classmethod
    def _work_row(cls, row: sqlite3.Row | None) -> dict[str, Any] | None:
        return cls._decode(row, json_fields=("dependencies", "result", "handoff"))
    ...
    def _validated_package(self, package: dict) -> list[dict]:
        ...
        work_fields = ("key", "title", "description", "classification", "dependencies")
        ...
        for specification_value in specifications:
            ...
            dependencies = self._string_list(
                specification["dependencies"], "specification dependencies"
            )
            ...
            for candidate in candidates:
                ...
                if dependencies:
                    dependency_rows = connection.execute(
                        "SELECT key, state FROM work_items WHERE pass_id = ?",
                        (candidate["pass_id"],),
                    ).fetchall()
                    states = {row["key"]: row["state"] for row in dependency_rows}
                    if any(
                        states.get(key) not in TERMINAL_WORK_STATES
                ...
                if specification_dependencies:
                    ...
                    id_placeholders = ", ".join("?" for _ in dependency_ids)
                    nonterminal = self._fetch_one(
                        connection,
                        f"""
                        SELECT id FROM work_items
                        WHERE specification_id IN ({id_placeholders})
                          AND state NOT IN ({terminal_placeholders})
                        LIMIT 1
                        """,
                        (*dependency_ids, *terminal_states),
                    )
    ...
    def _validated_handoff(self, handoff: dict) -> dict:
        handoff = self._require_fields(
            handoff,
            ("classification", "context", "artifacts", "dependencies", "blocking"),
            "handoff",
        )
        ...
        dependencies = self._string_list(handoff["dependencies"], "handoff dependencies")
