...
function renderReadyIssues(repository) {
  ...
  if (!discovery || discovery.status === 'unavailable') {
    message = `<p class="error">Ready-issue discovery is unavailable${discovery?.error ? `: ${esc(discovery.error)}` : '.'}</p>`;
  } else if (discovery.status === 'stale') {
    message = `<p class="error">Ready-issue inventory is stale and is not presented as current${discovery.error ? `: ${esc(discovery.error)}` : '.'}</p>`;
  } else if (!issues.length) {
    message = '<p class="muted">No agent:ready issues for this repository.</p>';
  }
  const cards = discovery?.status === 'available'
    ? issues.map(issue => `<article class="card"><div class="row"><strong>${githubLink(issue.url, `Issue #${issue.number}: ${issue.title}`)}</strong><span class="badge">agent:ready</span></div><p class="muted">Updated ${esc(issue.updated_at)}</p></article>`).join('')
    : '';
